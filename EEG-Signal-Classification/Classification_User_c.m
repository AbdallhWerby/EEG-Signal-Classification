...
if any(mat_ccc)
    % Remove the rows
    Matrix_c = User_c(~mat_ccc,:);
end

Matrix_c = table2array(Matrix_c);
...
% Iterate over rows of Matrix_c in 160-sample intervals
for i = 1:160:size(Matrix_c,1)-159
% Extract 1 second of data for delta waves from Matrix_c
...
for j = delta_col_indices % iterate over columns containing delta data
    delta_data_second = [delta_data_second Matrix_c(i:i+159, j)']; % select rows i to i+159 and column j, transpose, and concatenate horizontally
end
delta_data_c = [delta_data_c; delta_data_second]; % concatenate vertically to add current second of delta data to the array

% Extract 1 second of data for theta waves from Matrix_c
...
for j = theta_col_indices % iterate over columns containing theta data
    theta_data_second = [theta_data_second Matrix_c(i:i+159, j)']; % select rows i to i+159 and column j, transpose, and concatenate horizontally
end
theta_data_c = [theta_data_c; theta_data_second]; % concatenate vertically to add current
% Extract 1 second of data for alpha waves from Matrix_c
...
for j = alpha_col_indices % iterate over columns containing alpha data
    alpha_data_second = [alpha_data_second Matrix_c(i:i+159, j)']; % select rows i to i+159 and column j, transpose, and concatenate horizontally
end
alpha_data_c = [alpha_data_c; alpha_data_second]; % concatenate vertically to add current second of alpha data to the array

% Extract 1 second of data for beta waves from Matrix_c
...
for j = beta_col_indices % iterate over columns containing beta data
    beta_data_second = [beta_data_second Matrix_c(i:i+159, j)']; % select rows i to i+159 and column j, transpose, and concatenate horizontally
end
beta_data_c = [beta_data_c; beta_data_second]; % concatenate vertically to add current second of beta data to the array
end
...
scaled_delta_data_c = normalize(delta_data_c);
scaled_theta_data_c = normalize(theta_data_c);
scaled_alpha_data_c = normalize(alpha_data_c);
scaled_beta_data_c = normalize(beta_data_c);
...
Waves = [scaled_delta_data_c scaled_theta_data_c scaled_alpha_data_c scaled_beta_data_c];
...
% Iterate over rows of Matrix_c in 160-sample intervals
for i = 1:160:size(Matrix_c,1)-159
Labels = [Labels Matrix_c(i, 1)]; % add value of the first column at row i to the array
end
Labels = Labels.';
...
x = 1:size(delta_data_c,2); % set x-axis values to be the indices of the columns in delta_data_c
...
for i = 1:size(delta_data_c,1)
    % Plot the i-th row of delta_data_c
    plot(x, delta_data_c(i,:));
end
...
hold on; % keep all of the plotted lines on the same graph
for i = 1:size(scaled_delta_data_c,1)
    % Plot the i-th row of scaled_delta_data_c
    plot(x, scaled_delta_data_c(i,:));
end
xlabel('Time (s)'); % set x-axis label
ylabel('Amplitude'); % set y-axis label
...
hold on; % keep all of the plotted lines on the same graph
for i = 1:size(theta_data_c,1)
% Plot the i-th row of theta_data_c
plot(x, theta_data_c(i,:));
end
xlabel('Time (s)'); % set x-axis label
ylabel('Amplitude'); % set y-axis label
...
hold on; % keep all of the plotted lines on the same graph
for i = 1:size(scaled_theta_data_c,1)
% Plot the i-th row of scaled_theta_data_c
plot(x, scaled_theta_data_c(i,:));
end
xlabel('Time (s)'); % set x-axis label
ylabel('Amplitude'); % set y-axis label
...
hold on; % keep all of the plotted lines on the same graph
for i = 1:size(delta_data_c,1)
% Plot the first half of the i-th row of delta_data_c
plot(x(1:size(delta_data_c,2)/2), delta_data_c(i,1:size(delta_data_c,2)/2));
end
xlabel('Time (s)'); % set x-axis label
ylabel('Amplitude'); % set y-axis label
title('Delta Data Before Scaling'); % set plot title
...
hold on; % keep all of the plotted lines on the same graph
for i = 1:size(scaled_delta_data_c,1)
% Plot the first half of the i-th row of scaled_delta_data_c
plot(x(1:size(scaled_delta_data_c,2)/2), scaled_delta_data_c(i,1:size(scaled_delta_data_c,2)/2));
end
xlabel('Time (s)'); % set x-axis label
ylabel('Amplitude'); % set y-axis label
title('Delta Data After Scaling'); % set plot title
