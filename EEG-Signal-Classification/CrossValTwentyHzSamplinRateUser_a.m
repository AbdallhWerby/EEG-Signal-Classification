...
% Iterate over rows of Matrix_a in 20-sample intervals
for i = 1:20:size(Matrix_a,1)-19
...
for j = delta_col_indices % iterate over columns containing delta data
    delta_data_second = [delta_data_second Matrix_a(i:i+19, j)']; % select rows i to i+19 and column j, transpose, and concatenate horizontally
end
delta_data_a = [delta_data_a; delta_data_second]; % concatenate vertically to add current second of delta data to the array

% Extract 1 second of data for theta waves from Matrix_a
...
for j = theta_col_indices % iterate over columns containing theta data
    theta_data_second = [theta_data_second Matrix_a(i:i+19, j)']; % select rows i to i+19 and column j, transpose, and concatenate horizontally
end
theta_data_a = [theta_data_a; theta_data_second]; % concatenate vertically to add current
% Extract 1 second of data for alpha waves from Matrix_a
...
for j = alpha_col_indices % iterate over columns containing alpha data
    alpha_data_second = [alpha_data_second Matrix_a(i:i+19, j)']; % select rows i to i+19 and column j, transpose, and concatenate horizontally
end
alpha_data_a = [alpha_data_a; alpha_data_second]; % concatenate vertically to add current second of alpha data to the array

% Extract 1 second of data for beta waves from Matrix_a
...
for j = beta_col_indices % iterate over columns containing beta data
    beta_data_second = [beta_data_second Matrix_a(i:i+19, j)']; % select rows i to i+19 and column j, transpose, and concatenate horizontally
end
beta_data_a = [beta_data_a; beta_data_second]; % concatenate vertically to add current second of beta data to the array
end
...
% Generate indices for splitting the data into training and test sets
cv = cvpartition(size(shuffledArrayNoLabels,1),'HoldOut',1-trainPercent);
idx = cv.test;
...
partition = cvpartition(Labels,'KFold',nfolds);
...
x = 1:size(delta_data_a,2); % set x-axis values to be the indices of the columns in delta_data_a
...
for i = 1:size(delta_data_a,1)
    % Plot the i-th row of delta_data_a
    plot(x, delta_data_a(i,:));
end
...
hold on; % keep all of the plotted lines on the same graph
for i = 1:size(scaled_delta_data_a,1)
    % Plot the i-th row of scaled_delta_data_a
    plot(x, scaled_delta_data_a(i,:));
end
xlabel('Time (s)'); % set x-axis label
ylabel('Amplitude'); % set y-axis label
...
hold on; % keep all of the plotted lines on the same graph
for i = 1:size(theta_data_a,1)
% Plot the i-th row of theta_data_a
plot(x, theta_data_a(i,:));
end
xlabel('Time (s)'); % set x-axis label
ylabel('Amplitude'); % set y-axis label
...
hold on; % keep all of the plotted lines on the same graph
for i = 1:size(scaled_theta_data_a,1)
% Plot the i-th row of scaled_theta_data_a
plot(x, scaled_theta_data_a(i,:));
end
xlabel('Time (s)'); % set x-axis label
ylabel('Amplitude'); % set y-axis label
...
hold on; % keep all of the plotted lines on the same graph
for i = 1:size(delta_data_a,1)
% Plot the first half of the i-th row of delta_data_a
plot(x(1:size(delta_data_a,2)/2), delta_data_a(i,1:size(delta_data_a,2)/2));
end
xlabel('Time (s)'); % set x-axis label
ylabel('Amplitude'); % set y-axis label
title('Delta Data Before Scaling'); % set plot title
...
hold on; % keep all of the plotted lines on the same graph
for i = 1:size(scaled_delta_data_a,1)
% Plot the first half of the i-th row of scaled_delta_data_a
plot(x(1:size(scaled_delta_data_a,2)/2), scaled_delta_data_a(i,1:size(scaled_delta_data_a,2)/2));
end
xlabel('Time (s)'); % set x-axis label
ylabel('Amplitude'); % set y-axis label
title('Delta Data After Scaling'); % set plot title
